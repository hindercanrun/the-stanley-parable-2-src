	...
	public class AmplifyImpostor : MonoBehaviour
	{
		...
		private void GenerateTextures(List<TextureOutput> outputList, bool standardRendering)
		{
			...
			if (standardRendering && this.m_renderPipelineInUse == RenderPipelineInUse.HD)
			{
				...
				this.m_rtGBuffers[0] = new RenderTexture((int)this.m_data.TexSize.x, (int)this.m_data.TexSize.y, 16, format);
				this.m_rtGBuffers[0].Create();
				this.m_rtGBuffers[1] = new RenderTexture((int)this.m_data.TexSize.x, (int)this.m_data.TexSize.y, 16, format2);
				this.m_rtGBuffers[1].Create();
				this.m_rtGBuffers[2] = new RenderTexture((int)this.m_data.TexSize.x, (int)this.m_data.TexSize.y, 16, format2);
				this.m_rtGBuffers[2].Create();
				this.m_rtGBuffers[3] = new RenderTexture((int)this.m_data.TexSize.x, (int)this.m_data.TexSize.y, 16, format3);
				this.m_rtGBuffers[3].Create();
				this.m_rtGBuffers[4] = new RenderTexture((int)this.m_data.TexSize.x, (int)this.m_data.TexSize.y, 16, format2);
		...
		private Matrix4x4 GetCameraRotationMatrix(ImpostorType impostorType, int hframes, int vframes, int x, int y)
		{
			...
			if (impostorType == ImpostorType.Spherical)
			{
				...
				Quaternion lhs = Quaternion.Euler(num * (float)y + 90f, 0f, 0f);
				Quaternion rhs = Quaternion.Euler(0f, 360f / (float)hframes * (float)x + -90f, 0f);
				result = Matrix4x4.Rotate(lhs * rhs);
			}
